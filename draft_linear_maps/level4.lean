...
/--
## Summary
In a finite-dimensional vector space `V`, any submodule (subspace) `U` has a dimension (finrank) that is less than or equal to the dimension of `V` (the ambient space).

## Key idea
The proof uses a linear map argument. Consider the natural inclusion map from `U` into `V` (in Lean, this is `U.subtype : U →ₗ[K] V`). This map is injective (different elements of `U` map to different elements in `V`), which Lean formalizes as `U.subtype_injective`. By applying the general theorem `LinearMap.finrank_le_finrank_of_injective` to this injective map, we conclude `Module.finrank K U ≤ Module.finrank K V`. Essentially, this is the formal way to say "a basis of `U` can be extended to a basis of `V`," so the size of a basis of `U` (the dimension of `U`) cannot exceed the size of a basis of `V`.
-/
...
TheoremDoc submodule_finrank_le_explicit_aux as "Subspace Dimension ≤ Space Dimension" in "BasisWorld"
